...
def compute_fid_score(
    ...
    for i in range(num_batches):
        ...
        if save_dir:
            ...
            print(f"Saving generated images to {fn}")
            np.save(fn, img)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_dir",
        help="Directory where the model files are (model config, EMA weights)",
        required=True,
        type=str
    )   
    parser.add_argument(
    ...
    parser.add_argument(
        "--num_images",
        help="Number of images to use (Default: 60000)",
        type=int,
        default=60000
    )
    parser.add_argument(
