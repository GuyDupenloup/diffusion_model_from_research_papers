...
I knew from the start that I would not have sufficient GPU resources at my disposal to tackle the CelebA and LSUN datasets (I used Google Colab), so I planned to only work with the CIFAR-10 dataset. But during the course of the project, I realized that I would not be able to train the same U-Net as in Ho's code. Therefore, I started with the MNIST dataset and went on with a scaled-down version of Ho's U-Net for CIFAR-10, although expectations were obviously lower.
...
```
    src
     ...
     |
     └── utils.py                   # Utility to display a model summary
```
...
DDPM used a linear beta schedule. Instead, I used the cosine schedule introduced by Nichol & Dhariwal in 2021. This type of schedule was shown to improve results in many settings as it avoids destroying image information too quickly at the beginning of the forward process, like observed with linear schedules.
...
Due to limited GPU resources, I could only train the model for 150 epochs.

### 7.3 Sampling

Examples of quality images I obtained are shown in Figure 6.

![](pictures/cifar10_quality_samples.png)

Only about one out of five of the generated images had acceptable quality. Diversity was fine, although cars and trucks were slightly over-represented.

Examples of flawed images are shown in Figure 7.

![](pictures/cifar10_flawed_samples.png)

Clearly, better results could have been obtained with a U-Net of the same size as in Ho's code and with more training epochs.
